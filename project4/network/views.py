...
class NewPostForm(forms.Form):
    post_text = forms.Field(widget=forms.Textarea(
        {'rows': '5', 'maxlength': 256, 'class': 'form-control', 'placeholder': "Start a post"}), label="New Post", required=True)


class NewEditPostForm(forms.Form):
    id_post_edit_text = forms.Field(widget=forms.Textarea(
        {'rows': '5', 'maxlength': 256, 'class': 'form-control', 'placeholder': "Start a post?", 'id': 'id_post_edit_text'}), label="New Post", required=True)
...
def editpost(request, id):
    ...
    return JsonResponse({"error": HttpResponseBadRequest("Bad Request: This page isn't working at the moment :(")}, status=400)
...
def follow(request, id):
    try:
        ...
        return HttpResponseBadRequest("Bad Request: This page isn't working at the moment :(")
...
def profile(request, username):
    ...
    return render(request, "network/profile.html", {
        "user_profile": profile_user, "posts": page_obj, "is_following": is_following, 'total_following': total_following,
         'total_followers': total_followers, 'form': NewPostForm(), 'form_edit': NewEditPostForm()
    })
